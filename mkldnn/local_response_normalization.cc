...
//     auto lrn_user_src_memory = new memory({{{lrn_src_tz}, memory_T<T>(),
//         memory::format::nchw}, cpu_engine}, x);
//   	auto lrn_dst_memory = new memory({{{lrn_src_tz}, memory_T<T>(),
//         memory::format::nchw}, cpu_engine}, y);
//   	auto lrn_scratch_memory = new memory(lrn_dst_memory.get_primitive_desc());

//     /* create memory descriptors*/
//     auto lrn_src_md = new memory::desc({lrn_src_tz}, memory_T<T>(),
//         memory::format::nchw);
//     auto lrn_dst_md = new memory::desc({lrn_dst_tz}, mmemory_T<T>(),
//         memory::format::nchw);
...
int LocalResponseNormalization<T>::backward(){
	...
	*diff_src);
	...
	// check_lrn_bwd<T>(p, *src, *diff_dst, *diff_src);
	return 0;
}
